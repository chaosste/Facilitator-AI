...
const AmbientPlayer: React.FC<AmbientPlayerProps> = ({ isOpen, onClose }) => {
  ...
  const updateVisualizer = () => {
    if (analyserRef.current) {
      ...
      // Extract a few bins for a simplified bar visualizer
      const simplified = [
        dataArray[2] || 0,
        dataArray[5] || 0,
        dataArray[8] || 0,
  ...
  return (
    <div className="absolute top-24 right-8 w-80 max-h-[75vh] bg-white/95 backdrop-blur-2xl border border-[#96adb3]/30 rounded-[3rem] shadow-2xl z-[100] p-8 animate-in fade-in zoom-in duration-500 flex flex-col">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={onClose} className="text-[#2c3e50]/30 hover:text-[#96adb3] transition-all p-2 hover:bg-[#96adb3]/10 rounded-full">
        ...
        {AMBIENT_TRACKS.map((track) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-6 p-6 rounded-[2.5rem] border transition-all duration-700 text-left relative overflow-hidden group ${
                active && isPlaying 
                  ? 'bg-white border-[#96adb3] shadow-md' 
                  : 'bg-white/40 border-transparent hover:border-[#96adb3]/20'
              }`}
            >
              <div className={`text-6xl transition-transform duration-1000 ${active && isPlaying ? 'scale-110' : 'group-hover:scale-105 opacity-40'}`}>
                {track.icon}
              </div>
              <div className="flex-1 space-y-1">
                <span className={`text-[11px] font-black uppercase tracking-widest block ${active && isPlaying ? 'text-[#2c3e50]' : 'text-[#2c3e50]/60'}`}>
                  {track.name}
                </span>
                <p className="text-[10px] font-light text-[#2c3e50]/40 leading-tight">
                  {track.description}
                </p>
              </div>
              ...
              {active && isPlaying && (
                <div className="flex gap-1 items-end h-6 w-10">
                  {freqData.map((val, i) => (
                    <div 
                      key={i} 
                      className="flex-1 bg-[#96adb3] rounded-full transition-all duration-75"

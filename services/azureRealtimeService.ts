...
export const connectAzureRealtimeSession = async (
  ...
  const tokenData = (await tokenResponse.json().catch(() => ({}))) as Partial<TokenResponse> & { error?: string };
  ...
  dataChannel.onopen = () => {
    ...
    dataChannel.send(
      JSON.stringify({
        ...
        response: { modalities: ['audio', 'text'], instructions: 'Begin with a concise supportive welcome.' }
      })
    );
  };
  dataChannel.onmessage = (event) => {
    try {
      ...
      if (message.type === 'error') {
        callbacks.onError(message?.error?.message || 'Realtime data channel returned an error.');
      }
    } catch {
